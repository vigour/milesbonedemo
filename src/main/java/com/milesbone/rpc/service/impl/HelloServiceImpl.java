package com.milesbone.rpc.service.impl;

import java.util.ArrayList;
import java.util.Calendar;
import java.util.Date;
import java.util.List;

import org.apache.commons.lang3.RandomStringUtils;

import com.milesbone.account.entity.User;
import com.milesbone.rpc.service.IHelloService;

/**
 * 
 * @author miles
 * @date 2017-06-11 上午12:24:14
 */

//@RpcService(IHelloService.class)//指定远程接口  使用 RpcService注解定义在服务接口的实现类上，需要对该实现类指定远程接口，因为实现类可能会实现多个接口，一定要告诉框架哪个才是远程接口。
public class HelloServiceImpl implements IHelloService {

	@Override
	public String hello(String name) {
		String result = "hello " + name;
		System.out.println(result);
		return result;
	}

	@Override
	public User getUser(String name) {
		User user = new User("aaa", name, new Date(), 10000D);
		return user;
	}

	@Override
	public List<User> getUsers(int size) {
		List<User> list = new ArrayList<>();
		User user = null;
		Date birthday = new Date();
		String userid = "aaa";
		String name = "foo";
		Calendar cal = Calendar.getInstance();
		cal.setTime(birthday);
		
		for(int i = 0; i < size; i++){
			cal.add(Calendar.DAY_OF_MONTH, 1);
			user = new User(userid + i, name + i, cal.getTime(), Double.parseDouble(RandomStringUtils.random(100000)));
			list.add(user);
		}
		return list;
	}

	@Override
	public User updateUser(User user) {
		user.setUserName(user.getUserName()+"-update");
		return user;
	}

}
